      ...
      <Col md={8} sm={24}>
        <div className={cx('list-document')}>
          <Row gutter={0} className={cx('list-card')}>
            ...
            <Col span={12}>
              <Link
                className={cx('search-right')}
                to={routes.publishedDocumentList}
              >
        ...
        <div className={cx('list-news')}>
          ...
          <div className={cx('list-document-content')}>
            <div className={cx('title-news')}>
              ...
              <Link to={commonRender.renderLinkNewsCategory(CategoryNews?.Id)}>
                {CategoryNews?.CategoryNewsName}
              </Link>
            </div>
            <Skeleton loading={isLoading} active>
              <>
                  ...
                  items.map((item) => {
                    return (
                      <ListSectionNews
                        ...
                        key={item.Id}
                        avatar={item.Avatar}
                        href={commonRender.renderLinkNewsDetail(item.Id)}
                      />
                    );
                  })}
              </>
