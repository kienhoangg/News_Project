...
function MenuPage(props) {
        ...
        return (
            <div onClick={() => handleClickNode(Id)} style={{ display: 'flex' }}>
                ...
                <div style={{ display: displayIcon[Id] ? 'block' : 'none' }}>
                    <InfoCircleOutlined title='Chi tiết' style={{ margin: '0 10px', padding: 4, cursor: 'pointer' }} onClick={() => handleShowDetail(Id)} />
                    <EditOutlined title='Sửa menu' style={{ margin: '0 10px', padding: 4, cursor: 'pointer' }} onClick={() => editMenu(Id)} />
                    <CheckSquareOutlined
                        title={Status ? 'Hủy duyệt' : 'Duyệt'}
                        style={{ margin: '0 10px', padding: 4, cursor: 'pointer', color: Status ? '#f81d22' : '#008000' }}
                        onClick={() => displayMenu(Id, Status)}
                    />
                    <CloseOutlined title='Xóa menu' style={{ margin: '0 10px', padding: 4, cursor: 'pointer' }} onClick={() => deleteMenu(Id, Status)} />
                </div>
            </div>
        );
    };
    ...
    const handleChangeView = (id) => {
        ...
        const dataFilter = dataResource.current.filter((x) => x.Id === id || x.ParentId === id);
        setDataTree(dataFilter);
    };

    const renderOption = (
        <Select placeholder='Chọn cấp cha' style={{ width: '100%' }} allowClear={true}>
    ...
    return (
        <div className={cx('wrapper')}>
            ...
            <Modal className={cx('modal-category-news')} title='Thêm mới danh mục tin' open={isModalOpen} onCancel={handleCancel} footer={null}>
                <Form
                    {...layout}
                    form={form}
                    name='control-hooks'
          ...
          <Form.Item name='isPublish' label='Xuất bản' valuePropName='checked'>
            <Checkbox />
                    ...
                    <Form.Item wrapperCol={{ offset: 8, span: 16 }}>
                        <Button type='primary' htmlType={mode.current === Mode.Edit ? 'Sửa' : 'Tạo mới'}>
            ...
            <div className={cx('top')}>
                <MenuSearch dataMenu={dataResource.current} changeParent={handleChangeView} />
                ...
                <Row gutter={8}>
                    <Col span={16}>
                        ...
                        <Row gutter={16} className={cx('row-item')}>
                            ...
                            <Col span={14}>
                                <div>{dataResource.current.find((x) => x.Id === detail.current?.ParentId)?.Title ?? ''}</div>
                            </Col>
                        </Row>
