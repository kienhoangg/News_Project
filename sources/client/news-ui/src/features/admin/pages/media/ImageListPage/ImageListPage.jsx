...
function ImageListPage(props) {
    ...
    const callApiGetDetailImage = async (id) => {
        ...
        try {
            ...
            form.setFieldsValue({
                Title: res?.Title,
                PhotoCategoryId: dataFilter?.categoryAll.find((x) => x.Id === res?.PhotoCategoryId)?.Title ?? '',
                Order: res?.Order,
            });
    ...
    const fetchCategoryList = async () => {
        try {
            ...
            openNotification('Lấy danh sách hình ảnh thất bại', '', NotificationType.ERROR);
        }
    };
    ...
    const onCreate = async (values) => {
        try {
            ...
            if (isModalOpen?.type === MODAL_TYPE.CREATE) {
                ...
                openNotification('Cập nhật hình ảnh thất bại', '', NotificationType.ERROR);
            }
        }
    };
    ...
    const onFinish = (values) => {
        ...
        if (PhotoCategoryId) {
            bodyData.PhotoCategoryId = parseInt(dataFilter?.categoryAll.find((x) => x.Title === PhotoCategoryId)?.Id ?? undefined);
        }
        ...
        for (let i = 0; i < fileUploadCurrent.length; i++) {
            ...
            if (file.size > LIMIT_UP_LOAD_FILE) {
                openNotification(`File thứ ${i + 1} đã lớn hơn 2MB`, '', NotificationType.ERROR);
                return;
            }
    ...
    return (
        <div className={cx('wrapper')}>
            {(isModalOpen?.type === MODAL_TYPE.CREATE || isModalOpen?.type === MODAL_TYPE.EDIT || isModalOpen?.type === MODAL_TYPE.DETAIL) && isModalOpen?.show ? (
                <Modal
                    open={true}
                    title={isModalOpen?.type === MODAL_TYPE.CREATE ? 'Tạo mới hình ảnh' : isModalOpen?.type === MODAL_TYPE.DETAIL ? 'Chi tiết' : 'Chỉnh sửa hình ảnh'}
                ...
                >
                    <Form form={form} {...layout} name='control-hooks' onFinish={onFinish}>
                        ...
                        >
                            {isModalOpen?.type === MODAL_TYPE.DETAIL ? <div>{isModalOpen?.imageDetail?.Title}</div> : <Input />}
                        </Form.Item>

                        <Form.Item label='Danh mục hình ảnh' name='PhotoCategoryId'>
                            {isModalOpen?.type === MODAL_TYPE.DETAIL ? (
                                <div>{dataFilter?.categoryAll?.find((item) => item?.Id === isModalOpen?.imageDetail?.PhotoCategoryId)?.Title}</div>
                            ) : (
                        ...
                        <Form.Item name='Order' label='Số thứ tự'>
                            {isModalOpen?.type === MODAL_TYPE.DETAIL ? <div>{isModalOpen?.imageDetail?.Order}</div> : <Input type='number' min={0} />}
                        </Form.Item>

                        <Form.Item name='lb-attachment' label='Tệp đính kèm'>
                            {isModalOpen?.type === MODAL_TYPE.DETAIL ? (
                                <div>
                                    {isModalOpen?.imageDetail?.ImagePath?.split(';;').map((item) => {
                                        return (
                                            <div
                                                href=''
                                                style={{
                                            ...
                                            >
                                                ...
                                                <a href={imageHelper.getLinkImageUrl(item)} style={{ marginLeft: 4 }} target='_blank'>
                ...
                <Button icon={<UploadOutlined />}>Tải lên Tệp</Button>
                        ...
                        {isModalOpen?.type === MODAL_TYPE.DETAIL && (
                            <Form.Item name='linkImage' label='Đường dẫn ảnh'>
                                {Array.isArray(isModalOpen?.imageDetail?.ImagePath?.split(';;')) &&
                                    isModalOpen?.imageDetail?.ImagePath?.split(';;').map((item, index) => {
                                        return (
                                            <Input.Group compact key={index}>
                                                ...
                                                />
                                                <CopyToClipboard text={imageHelper.getLinkImageUrl(item)}>
