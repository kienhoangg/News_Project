          ...
          onOk={() => {
              form.validateFields()
                  .then((values) => {
                      ...
                      const { collaboratorId, category, title, IsDocumentNews, IsNewsHot, IsNewsVideo, IsDisplayTitle, IsDisplayAvatar, IsComment, avatarTitle, description, content, field, source } =
                          values;
                      const bodyData = {
                          Title: title,
                      ...
                      if (field) {
                          bodyData.FieldNewsId = dataFilter?.fieldNews.find((x) => x.Title === field)?.Id ?? undefined;
                      }
                      if (source) {
                          bodyData.SourceNewsId = dataFilter?.sourceNews.find((x) => x?.Title === source)?.Id ?? undefined;
                      }
                      if (category) {
                          bodyData.CategoryNewsId = dataFilter?.categoryNews.find((x) => x?.CategoryNewsName === category)?.Id ?? undefined;
                      }
                      if (collaboratorId) {
                          bodyData.CollaboratorId = dataFilter?.collaborators.find((x) => x?.Name === collaboratorId)?.Id ?? undefined;
                      }
      ...
      >
          ...
          >
              ...
              <Form.Item label='Tin nổi bật'>
                  <Row gutter={8}>
                      <Col span={4}>
                          <Form.Item style={{ marginBottom: 0 }} name='IsDocumentNews' valuePropName='checked'>
                              <Checkbox></Checkbox>
                          </Form.Item>
                      </Col>
                      {/* <Col span={4}>
