...
function StaticContentDetail(props) {
    ...
    const getLinkFileAttachment = (filePath = '') => {
        return filePath
            ? filePath?.indexOf('https://') === 0 || filePath?.indexOf('http://') === 0
                ? filePath
                : (window.location.origin === 'http://localhost:3000' ? 'https://localhost:7122/' : window.location.origin) +
                  (filePath.startsWith('/wwwroot/', '') ? filePath.replaceAll('/wwwroot/', '') : filePath)
            : '/';
    };

    const getNameFile = (filePath = '') => {
        const file = filePath?.replaceAll('/wwwroot/UploadFiles/FileAttachments/', '');
        const fileName = file.substring(0, file.lastIndexOf('_')) + file.substring(file.lastIndexOf('.'), file.length);
                        ...
                        <Col span={20}>
                            <div>
                                <img
                                    ...
                                    src={
                                        data?.Avatar
                                            ? data?.Avatar?.indexOf('https://') === 0 || data?.Avatar?.indexOf('http://') === 0
                                                ? data?.Avatar
                                                : (window.location.origin === 'http://localhost:3000' ? 'https://localhost:7122/' : window.location.origin) +
                                                  (data?.Avatar.startsWith('/wwwroot/', '') ? data?.Avatar.replaceAll('/wwwroot/', '') : data?.Avatar)
                                            : '/'
                                    }
                                />
                            </div>
                    ...
                    <Row gutter={16} className={cx('row-item')}>
                        ...
                        <Col span={20}>
                            {getNameFile(data?.FilePath) && (
                                <div className={cx('file-attachment')} onClick={() => window.open(getLinkFileAttachment(data?.FilePath))}>
